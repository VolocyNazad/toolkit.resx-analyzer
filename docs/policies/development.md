...
Keep `CHANGELOG.md` up to date, in [Keep a Changelog](https://keepachangelog.com/en/1.1.0/) format - add an entry under `## [Unreleased]` (in the right category: Added/Changed/Fixed/Removed/...) as part of the same change, not as a separate follow-up. At release time, rename `[Unreleased]` to the version being tagged (matching the same version used in `AnalyzerReleases.Shipped.md` and the git tag) and start a fresh empty `[Unreleased]` heading above it.
...
The distributed analyzer targets Microsoft.CodeAnalysis 4.14, matching Visual Studio 2022 version 17.14. Keep Roslyn references private implementation dependencies and verify analyzer tests before changing this compatibility baseline.

## Test execution

Tests use xUnit v3 4.0 and run through Microsoft.Testing.Platform, selected in global.json. CI invokes the test project explicitly and treats a run with no discovered tests as a failure.
